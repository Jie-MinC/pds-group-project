...
model <- lm(resale_price ~ polym(floor_area_sqm,  remaining_lease, max_floor_lvl, degree = i, raw = TRUE), data = p_reg_train_set)
...
```{r}
# Choose degree = ?, seems like higher better
poly_reg_model <- lm(resale_price ~ polym(floor_area_sqm,  remaining_lease, max_floor_lvl, degree = 7, raw = TRUE)+., data = p_reg_train_set)
...
ts.data = data %>%group_by(date) %>% summarise(mean_resale_price= mean(resale_price))
head(ts.data)

ts.data %>%
  ggplot(aes(x = date, y = mean_resale_price)) +
      geom_point(color = "darkorchid4") +
      labs(title = "Monthly resale price",
           y = "Resale Price",
